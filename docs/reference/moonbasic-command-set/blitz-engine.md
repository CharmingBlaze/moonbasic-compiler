...
| World | `CREATEWORLD()`, `CLEARWORLD()`, `SETAMBIENT()`, `SETFOG()`, `SETWIREFRAME()` — use **`Entity.Update(dt)`** + **`Camera.Begin()`**/`End()` + **`DrawEntities()`** instead of Blitz **UpdateWorld** / **RenderWorld** ([BLITZ3D.md](../BLITZ3D.md)). |
