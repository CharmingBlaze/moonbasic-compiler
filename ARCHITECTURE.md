...
- **gopls / build tags**: **`.vscode/settings.json`** sets **`fullruntime,gopls_stub`** plus **`CGO_ENABLED=1`** so Raylib **`*_cgo.go`** files analyze on Windows. **`gopls_stub`** switches **`runtime/terrain`** to its **stub** sources for the language server only (see **`//go:build`** on **`heap_objects_stub.go`** vs **`heap_objects_raylib.go`**); **`go build -tags fullruntime`** does **not** use **`gopls_stub`**. **`main.go`** / **`cmd/moonbasic`** use **`!fullruntime`** — you may see **“no packages found”** for those until you temporarily adjust gopls tags (**[`docs/DEVELOPER.md`](docs/DEVELOPER.md)**). **Jolt Linux sources** match only **`GOOS=linux`**; on Windows/macOS, gopls may hint for those files—use WSL/Linux or ignore.
