...
The orchestration logic lives in the `compiler/pipeline` package (`compile.go`, `registry.go`, `runner.go`, …). The CLI driver (`main.go`) must be a thin wrapper around these functions.
...
- **Registration order** in `compiler/pipeline/registry.go`: all **`RegisterModule`** calls **before** **`RegisterFromManifest`**
...
- **Registration**: New modules are **`RegisterModule`**’d in **`compiler/pipeline/registry.go`** in dependency order, all **before** **`RegisterFromManifest`**.
...
- **Registration**: In **`compiler/pipeline/registry.go`** / **`registry_physics.go`**, **`RegisterModule`** for **`charcontroller`**, **`physics2d`**, then **`physics3d`** (all **before** **`RegisterFromManifest`**) so natives override manifest stubs. **`charcontroller`** is registered **before** **`physics3d`** so **`Registry.Shutdown`** tears down **Jolt `CharacterVirtual`** instances **before** the physics world is destroyed. **`physics3d.NewModule().SetUserInvoker(vm.CallUserFunction)`** wires **`PHYSICS3D.PROCESSCOLLISIONS`** to user **`FUNCTION`** callbacks (queued events only; Jolt contact → queue is not fully wired yet).
...
- **Dependency**: **[`github.com/codecat/go-enet`](https://github.com/codecat/go-enet)** via **`replace` → [`third_party/go-enet`](third_party/go-enet)** (CGO). **ENet C sources are vendored and statically linked** on **Windows, Linux, and macOS** — release **`moonrun`** must **not** require system **`libenet.so`**. Call **`NET.START`** before creating hosts; **`NET.STOP`** (or registry **`Shutdown`**) destroys open hosts and **`enet_deinitialize`**.
