...
func (m *Module) cursorIsEnabled(args []value.Value) (value.Value, error) {
	if len(args) != 0 {
		return value.Nil, fmt.Errorf("CURSOR.ISENABLED expects no arguments")
	}
	return value.FromBool(!rl.IsCursorHidden()), nil // Raylib doesn't have IsCursorEnabled, but usually Lock+Hide go together.
}
