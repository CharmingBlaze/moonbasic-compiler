...
func registerWallClock(reg runtime.Registrar) {
	...
	dateWall := func(rt *runtime.Runtime, args ...value.Value) (value.Value, error) {
		if len(args) != 0 {
			return value.Nil, errArgs(0, len(args))
		}
		return rt.RetString(time.Now().Format("2006-01-02")), nil
	}
	reg.Register("DATE", "time", dateWall)
	reg.Register("DATE$", "time", dateWall)
	timeWall := func(rt *runtime.Runtime, args ...value.Value) (value.Value, error) {
		if len(args) != 0 {
			return value.Nil, errArgs(0, len(args))
		}
		return rt.RetString(time.Now().Format("15:04:05")), nil
	}
	reg.Register("TIME", "time", timeWall)
	reg.Register("TIME$", "time", timeWall)
	...
	datetimeWall := func(rt *runtime.Runtime, args ...value.Value) (value.Value, error) {
		if len(args) != 0 {
			return value.Nil, errArgs(0, len(args))
		}
		return rt.RetString(time.Now().Format("2006-01-02 15:04:05")), nil
	}
	reg.Register("DATETIME", "time", datetimeWall)
	reg.Register("DATETIME$", "time", datetimeWall)
}
