...
func NormalizeMethod(mn string) string {
	switch mn {
...
func RegistryPrefix(tag uint16) string {
	switch tag {
	case heap.TagCamera:
		return "CAMERA."
...
func Builtin(tag uint16, method string) (registryKey string, prependReceiver bool, ok bool) {
	mn := NormalizeMethod(strings.ToUpper(strings.TrimSpace(method)))
	switch tag {
	case heap.TagCamera:
		switch mn {
		case "END":
...
func Dispatch(tag uint16, method string, argCount int) (registryKey string, prependReceiver bool, ok bool) {
	if argCount > 0 {
		return Builtin(tag, method)
	}
	mn := NormalizeMethod(strings.ToUpper(strings.TrimSpace(method)))
	switch tag {
	case heap.TagCamera:
